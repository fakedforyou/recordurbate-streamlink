...
FROM python:3.11
WORKDIR /
RUN apt-get -y update && mkdir /source && mkdir -p /root/.local/share/streamlink/plugins && mkdir /recordurbate-streamlink && mkdir /recordurbate-streamlink/videos && apt-get -y install apt-utils && pip install streamlink==5.2.1 && pip install requests
